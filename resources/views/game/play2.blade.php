...
<style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    body {
        ...
        background: linear-gradient(rgba(0,0,0,0.5), rgba(0,0,0,0.5)), url("https://cdnb.artstation.com/p/assets/images/images/020/204/921/original/veronica-norbrink-cave-background-4-space-boii.gif?1566836439") no-repeat center center fixed;
    ...
    .battle-character-sprite { width: 220px; height: 220px; object-fit: contain; position: relative; z-index: 5; display: none; }
    ...
    .actions-menu {
        ...
        border-radius: 12px;
        padding: 20px 30px; /* ALTERADO: Aumenta o padding horizontal de 20px para 30px */
        z-index: 1;
    }
    .actions-grid {
        display: grid;
        ...
        color: #333;
        /* ALTERADO: Aumenta o espaçamento vertical entre as linhas */
        gap: 25px 20px; /* (vertical | horizontal) */
    }
    .action-item {
        font-size: 1.2rem;
        cursor: pointer;
...
const Game = {
    ...
    async gameOver(isVictory) {
        ...
        const saveData = { ...this.activePokemon, gold: this.state.trainer.gold, inventory: this.state.inventory };
        if (isVictory) {
            this.logMessage('FASE CONCLUÍDA!', 'log-lvlup');
            await this.saveProgress(saveData);
            this.logMessage('AVANÇANDO PARA A PRÓXIMA ÁREA...', 'log-system');
            setTimeout(() => { window.location.href = "{{ route('character.shop', ['character' => $character->id, 'next_stage' => 'play3']) }}"; }, 1500);
        } else {
